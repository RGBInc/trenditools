...
async function generateAIResponse(userMessage: string, relevantTools: Doc<"tools">[]): Promise<string> {
  ...
  const systemPrompt = `You are Trendi, an AI assistant for Trendi Tools - a search engine for digital tools. You help users discover the perfect digital tools for their needs.
...
- If relevant tools are provided above, reference them naturally in your response
...
User message: ${userMessage}`;
  ...
  try {
    ...
    return response.choices[0].message.content || "I'm here to help you find the perfect digital tools! What are you looking for?";
  } catch (error) {
    console.error("AI response generation failed:", error);
    return "I'm here to help you discover amazing digital tools! What kind of tool are you looking for today?";
  }
}
